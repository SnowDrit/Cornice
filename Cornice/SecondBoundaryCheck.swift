...
enum SecondBoundaryCheck {
    ...
    private static func finish(_ report: String, _ probes: [Probe]) async {
        log.info("\(report, privacy: .public)")
        writeReport(report, named: "two-wide-check.txt")
        ...
        // macOS writes each item's position under its autosave name as it lays out. These
        // are the check's own names, not the product's, and they have no business
        // outliving the check.
